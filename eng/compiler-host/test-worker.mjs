...
try {
  ...
  for (const source of sources) {
    ...
    if (result.application) {
      const module = Buffer.from(result.application, 'base64');
      writeFileSync(`application-${results.length}.wasm`, module);
      result.applicationBytes = module.length;
      result.applicationSha256 = createHash('sha256').update(module).digest('hex');
  ...
  if (!results[0].success || !results[1].success || results[2].success || !results[3].success ||
      ...
      results[2].bytes !== undefined || results[2].applicationBytes !== undefined ||
      !results[0].applicationSha256 || !results[1].applicationSha256 || !results[3].applicationSha256 ||
      results[0].applicationSha256 === results[1].applicationSha256 ||
      results[1].applicationSha256 === results[3].applicationSha256 || errors.length !== 0) {
  ...
  if (!editorResult.success || await page.locator('#status').textContent() !== 'Wasm compilation succeeded') {
  ...
  writeFileSync('worker-test.json', JSON.stringify(record, null, 2));
  console.log(`PASS: Chromium ${record.browser}; three distinct managed PEs and application modules; error then recovery`);
} catch (error) {
